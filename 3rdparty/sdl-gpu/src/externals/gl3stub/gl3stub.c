...
#ifndef SDL_GPU_DYNAMIC_GLES_2
#include <EGL/eglplatform.h>
#endif

#ifndef SDL_GPU_COMPILING_GLSTUB
#define SDL_GPU_COMPILING_GLSTUB
#endif
...
#ifdef _WIN32
    #define GL3STUB_GLES2_LIBRARY "libGLESv2.dll"
    #define GL3STUB_EGL_LIBRARY   "libEGL.dll"
#else
    #define GL3STUB_GLES2_LIBRARY "libGLESv2.so"
    #define GL3STUB_EGL_LIBRARY   "libEGL.so"
#endif
...
static void *(* EGLAPIENTRY getEglProcAddress)(const char *procname);

#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wpedantic"

static_inline void *getGles3ProcAddress(const char *sym) {
    void *proc = SDL_LoadFunction(gles2handle, sym);
    if (!proc && getEglProcAddress)
        proc = getEglProcAddress(sym);
    return proc;
}

static GLboolean gl3stubInit() {
#ifndef SDL_GPU_DYNAMIC_GLES_2
    // Looks like certain functions should be grabbed from GLESv2 library
    gles2handle = SDL_LoadObject(GL3STUB_GLES2_LIBRARY);
#endif
    eglhandle = SDL_LoadObject(GL3STUB_EGL_LIBRARY);
    getEglProcAddress = SDL_LoadFunction(eglhandle, "eglGetProcAddress");

    #define FIND_PROC(s) s = getGles3ProcAddress(#s)
